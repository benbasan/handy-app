...
- [ ] **Waiving the 35 ₪ fee on a customer a pro brought in themselves.** Opened on 14.9.2026, when the user chose Phase 13.8's personal pro link (`/new-request?pro=<slug>`) and said a waiver is worth considering. The argument for it: a pro is being asked to hand Handy a customer who was already theirs, and a fee on that customer is the reason they would not. It is money, so section 8 keeps it with the user, and three questions come before any code: **what counts as "a customer I brought"** (the call was created through the link, and nothing else); **forever, or the first job only**; and **how it is not gamed** — a pro who met a customer through the feed and then sends them the link to skip the fee on the next job. If it is taken, the lever is `job_acceptance_fee()` taking the job and the pro, with `accept_job()` still reading the amount from it and never from the caller. Phase 13.8 builds the link either way; it closes with this entry still open and no waiver code if the questions are not answered
- [ ] **An AI model in the job-posting path, or none.** Opened on 14.9.2026: the user is open to a photo → category + draft description step, subject to cost. The estimate before measuring is roughly 2 agorot per call on Claude Haiku 4.5, 4–5 on Claude Sonnet 5 and 10–15 on Claude Opus 5 — under one percent of the fee even at the top, so cost is unlikely to be what decides. What will decide is a third party in the critical path, a photo from inside somebody's home leaving for a processor, and an anonymous visitor (Phase 13.7 opens the form) running up calls. Phase 13.8 answers it with a hand-run measurement, `scripts/photo-intake-spike.mjs`, on 20 real photos — never wired into the product. If it is taken, it is a section 2 row, a flag, explicit consent before the photo is sent, a rate limit, a form that works unchanged when the service is down, and a clause in `lib/content/legal.ts`; the model only ever **fills** the form, it never publishes
