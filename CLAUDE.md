...
| Maps / geocoding / distance | **The built-in gazetteer**, with Google Maps Platform as the upgrade path | **The app runs with no Google key by choice** (decided 9.9.2026 — see section 9). `lib/maps/localities.data.ts` holds 313 Israeli localities generated from Wikidata by `scripts/build-gazetteer.mjs`; `lib/maps/gazetteer.ts` matches a typed address against them by whole words. That is locality-level, which is what a 10–30 km service radius needs. **Every deployed environment must set `ALLOW_NO_MAPS_KEY=1`** or a production build refuses every address. When a key does arrive there are **two**: `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` (browser, referrer-restricted) and `GOOGLE_MAPS_SERVER_API_KEY` (server, IP-restricted — a referrer-restricted key cannot geocode) |
...
- **A job reaches a pro through `pro_reaches_job()`, and that is the whole rule** — enforced in the RLS policy on `jobs` (and in `can_bid_on_job()`, `can_read_job_media()`, `record_job_view()` and the feed), not in a query, so it holds for anything that ever reads the table. It has three branches, in order. **(1) The pro the customer asked for by name** (`jobs.requested_pro_id`, Phase 13.8) reaches that call wherever they are, for as long as they are verified — even outside their radius, because the customer named them: the radius decides what the feed brings, never who a customer may ask for (decided 14.9.2026). **(2) Nobody else** reaches a directed call until it is opened, which happens only through `release_directed_job()`: the requested pro's "לא מתאים לי", their `decline_job()`, or the customer's `open_job_to_all()` — so "theirs until they pass" can never mean "stuck for ever". **(3) Everything else** is `pro_serves_job(location)`, the pro's own radius. It used to be `least(pro_profiles.radius_km, jobs.search_radius_km)` with the customer picking their half; that half was removed on 11.9.2026, column and all, because a customer does not know how far a plumber will drive. **The customer's only lever over reach is naming a pro** — which is why the offers screen, when nobody covers an undirected call, states the fact and offers no button.
- **A bid's two clocks and its status are not the form's to set.** `bids` grants INSERT on seven columns only (`job_id`, `pro_id`, `price`, `eta_minutes`, `note`, and since Phase 13.7 `arrival_window_start`/`arrival_window_end`, which are the pro's own hours exactly as `price` is their own number) — `expires_at`, `accept_deadline` and `status` are left to the database — and `status` has no UPDATE grant at all. Every transition is a `security definer` function: `select_bid()`, `accept_job()`, `decline_job()`, `withdraw_bid_selection()`, `expire_stale_bids()`, `expire_stale_selections()`. **One clock replaces the other:** an offer lapses at `expires_at`, but once it is chosen the pro has two hours (`bid_accept_window()`) and the offer must not lapse underneath them — the sweeps only ever touch the status each one owns. Both deadlines are re-checked on read and inside the functions, so they hold whether or not a cron sweep has run.
- **Opening a screen is not opening a permission.** `/new-request` left the `(authed)` group in Phase 13.7, so a visitor writes a whole call before being asked for a phone — and `createJob` still calls `requireRole("customer")`, and no policy on `jobs` or `job-media` moved. The two consequences that had to be handled rather than assumed: photos a visitor picks wait in the tab and upload under their own id after sign-in (`uploadPendingMedia()`), and any server action the open form calls must answer a non-customer with null, not `requireRole()` — a redirect from inside a server action navigates somebody half-way through a description to the login page (`countProsCovering()`).
- **A customer learns how many pros looked, never who.** `job_views` has no client grant at all; `record_job_view()` writes it and `job_view_count()` returns a number to the owner. The screen shows the number only while no offer has arrived — beside an offer, "5 צפו" teaches the customer to distrust the only one they have.
- **A BEFORE trigger that enforces a rule steps aside for a caller RLS is about to refuse.** Triggers run ahead of the insert policy, so `bids_check_arrival_window` first returns early unless the caller is the verified pro inserting their own bid. Without that, a customer's forged bid was answered "arrival window required" instead of 42501 — the wrong error, and a rule described to somebody with no business knowing it. pgTAP caught it by expecting 42501.
- **Adding to a call appends; it never rewrites.** `add_job_details()` works only while a call is `open`/`bidding` and adds after what pros already read, because an offer made against one description must not come to mean another. **No client role holds an UPDATE grant on `jobs` at all** (revoked 15.9.2026, with the policy that scoped it): a description a pro priced, a location that decided who was told, a `preferred_time` that decided whether an offer needed a window — none of them may move after posting except through a definer function that knows the call's state.
...
- **The fee is charged by `accept_job()`, is read from `job_acceptance_fee_for(job, pro)`, and is never refunded.** `job_fees` has never had an INSERT grant for any client role, and the amount is read from the function rather than accepted from the caller. It is the flat `job_acceptance_fee()` in every case but one, decided with the user on 14.9.2026: **0 on the first job of a new customer who came through that pro's own link** — "new" meaning no earlier job of theirs was ever accepted by anybody, which is also what stops it being gamed. The two-argument function has no client grant; a pro asks `my_fee_for_job()` about themselves, and only the requested pro can ever get anything but 35 back. Accepting is idempotent — a retried tap on a phone must not charge twice. Not refunding is a decision the user made on 8.9.2026 and it is why `resolve_dispute()` does not touch the ledger: a credit to the customer and the fee the pro paid to take the job are two different questions.
...
- **A number that sits beside a price has a sample floor.** A response time (`pro_response_minutes()`) needs three offers and a price range on the posting form (`MIN_PRICE_SAMPLE`) three closed jobs; below that the screen says nothing rather than something small. One lucky offer reading as "answers in 2 minutes", on the card a customer is about to choose from, is an invented figure with a real row behind it.
...
- **The gazetteer never invents a point.** An address naming no locality anybody recognises comes back as `null` and the customer is asked for a city — it used to come back as the middle of Tel Aviv under a `source` no screen read, so a call in an unrecognised town was broadcast to the wrong pros with nothing to say so. The matcher reads the **last comma-separated part first**, which is the same part `job_city()` reads in SQL (`split_part(address_text, ',', -1)`), so the stored point and the console's idea of that job's city agree by construction rather than by luck. `addressToStore()` appends the recognised town when the typed address does not already end with it, which is why the console stopped grouping jobs under "דירה 4".
- **A point from the browser — GPS, a saved address, Places Autocomplete — is a hint, and is re-checked in the server by `coordinatesInIsrael()` before it reaches a `geography` column.** It is preferred over parsing a string precisely because it is exact; it is never trusted because it is a value the client chose.
- **The gazetteer's data is generated, never typed.** A coordinate is a figure, and section 3's "no invented figure" covers 313 of them. `skos:altLabel` is where the spellings people actually write come from ("פתח תקוה" beside "פתח תקווה") — but it also carries nicknames, and Jerusalem's include "אריאל", which is a different city. So an alias is kept only where it is demonstrably a spelling **of the name**, and the gershayim forms are matched only against input somebody punctuated that way, because bare "פת" is a Hebrew word.
...
- **Depth is three tokens and a card spends the second one rarely.** `shadow-card`, `shadow-lift`, `shadow-overlay`, tinted with the ink hue because a neutral-black shadow over a blue-grey canvas reads as dirt. `CARD_BASE` carries the first; `CARD_RAISED` is for the card that *is* the screen's decision — the pro's acceptance card, the price-update approval, the recommended offer — and a screen with two of them has none. The border survives the shadow rather than being replaced by it: `bg-surface` is `#ffffff` over a `#f7f9fc` canvas, and a shadow soft enough not to look like a bubble cannot separate those two on its own.
- **An icon is drawn, never quoted.** `components/ui/icons.tsx` is the whole set: 24×24, no fill, `currentColor` at 1.75, size from the caller. Emoji were the rule until Phase 13.5 and the reason they are not is `currentColor` — a selected category tile signals selection by changing its border, its ground *and* its text, and the one element in the middle of it never changed. They are also a different picture per platform, on the strip that is the first thing a visitor sees. A category icon is `<CategoryIcon slug={…} />`, a component: returning a component *value* from a lookup is a new identity every render, so React remounts rather than updates, and `react-hooks/static-components` refuses it.
- **Dimming a card is not a style — it is a filter over every colour on it.** `opacity-70` on a lapsed offer took `text-muted` from 4.51:1 to 2.73:1 and the verified badge from 4.65:1 to 3.27:1. State that a row is inactive by changing its ground to `bg-canvas`, and leave the text alone; the status pill is what says the status. `e2e/a11y.spec.ts` catches this on the offers screen.
- **Motion is two named animations, and the `prefers-reduced-motion` block at the foot of `globals.css` is what makes having them safe.** `enter` is for something that genuinely appears — a toast, the phone's publish bar. It is deliberately not on the bid and feed cards: those are server-rendered lists, so it fired on every navigation rather than on arrival, and its `opacity: 0` start made axe measure text inside a card against the page canvas.
- **A phone gets a tab bar, and every screen says which one it is.** `MobileNav` below `md`, four destinations plus a "עוד" sheet, in all three shells; `NavLink` marks the active route above it. `exact` is load-bearing: `/pro` is the public landing page and `/pro/dashboard` the signed-in home, so a prefix match on the former lights up on every screen a pro sees. A count belongs **inside** its link — moving it to a sibling span looks identical and costs a screen-reader user the number.
- **A two-column screen pins its stacking order at every breakpoint, not at `lg`.** Thirteen screens carried `lg:order-*` with no base order, so on a phone they opened with the sidebar — the pro's feed led with a Maps placeholder and a tip card, and the jobs began below the fold. The desktop order is the intended reading order; give it a matching base `order-*`.
- **44px is the floor for anything a finger presses.** `BUTTON_COMPACT` (`min-h-11`) is the only weight allowed to be smaller than `BUTTON_BASE`, and `px-4 py-2 text-sm` on its own measured 34px — on "פרסם קריאה" and "פיד קריאות", the primary action of each side of the marketplace.
- **A value that has a name in `primitives.tsx` is used by that name.** The module opens by promising that a re-skin is one edit there rather than a sweep through every component; for `BUTTON_CTA` and `INPUT_CLASS` that was true, and for the card it was not — `CARD_CLASS` was imported by nobody while the three utilities that make a card look like a card were written out inline fifty-five times across thirty-five files. `CARD_BASE` is the card without its padding, because the app draws it at four different insets and which inset a card takes is a judgement against `design/screens/`, not something to unify from outside. Phase 13.5 added `CARD_RAISED`, `HERO_TITLE`, `PAGE_LEAD`, `SECTION_TITLE` and `BUTTON_COMPACT` on the same grounds, each one a duplication the audit found rather than a value invented for it — `mt-2 text-muted` was written out **43 times**, `text-lg font-bold text-ink` **59**, and the `<h1>` had drifted into six spellings. About half the first group turned out to be the second line of a hand-drawn empty state rather than a page lead, which is why eleven of those are now `EmptyState` instead. `tests/designTokens.test.ts` fails on any of the named literals reappearing outside that module.
...
| אזור פעילות / רדיוס | `service_area` / `radius_km` | The pro's own service radius, and since 11.9.2026 the **only** radius in the product — a job has none |
...
| חלון האישור | `bid_accept_window()` · `bids.accept_deadline` | Two hours. It was chosen when nothing notified a pro whose tab was closed; since Phase 13 something does (`bid_selected`, and `selection_expiring` thirty minutes out), and the window stays two hours anyway — a push is best-effort, a permission can be declined, and iOS needs a home-screen install |
...
| זיהוי יישוב | `matchLocality()` · `lib/maps/gazetteer.ts` | The town inside a typed address, by whole words. `null` is a real answer |
| היישוב הקרוב לנקודה | `nearestLocality()` | Turns a GPS fix into a name. The point itself is what gets stored |
| רשימת היישובים | `lib/maps/localities.data.ts` | Generated from Wikidata by `npm run gazetteer:build`. Never edited by hand |
| כתובות שמורות | `saved_places` | A customer's own addresses ("בית", "עבודה"). `customer_id` has no client grant and defaults to `auth.uid()` |
| מדריכי תחזוקה | `lib/content/guides.ts` | Articles in code, not a table: a guide restates rules that live in migrations |
| התראות | `notifications` | One thing that happened, addressed to one person. No client role may INSERT; `read_at` is the only writable column and a trigger pins it to `now()`, one-way. Carries **no Hebrew** — the copy is `lib/notifications/messages.ts` |
| סוג התראה | `NotificationKind` | Closed vocabulary, mirrored in `lib/notifications/kinds.ts` and in a `check` on the column. A Vitest assertion reads the migration to keep them identical |
| מרכז ההתראות | `/pro/notifications` · `/account/notifications` | design/screens/pro-5.4. The customer's sits under `/account` so `PROTECTED_AREAS` already covers it |
| מנוי דחיפה | `push_subscriptions` | One row per device. Written only through `save_push_subscription()`, which re-owns an endpoint a shared browser handed to a second account. **No admin read policy** — an endpoint is a capability, not a record |
| שליחת הדחיפה | `dispatch_pending_pushes()` → `/api/notifications/dispatch` | pg_cron + pg_net to a route that holds **no database credential**: the batch carries everything a channel needs |
| אזהרת פקיעה | `warn_expiring_selections()` · `bids.lapse_warned_at` | Warns a pro once, thirty minutes before the two-hour window lapses. Housekeeping — `accept_job()` re-reads the deadline itself |
| צפיות בקריאה | `job_views` · `record_job_view()` · `job_view_count()` | Which verified pros opened a call, once each. No client grant; the owner gets a count, never an identity |
| הוסיפו פרטים לקריאה | `add_job_details()` | Appends text and photos while `open`/`bidding`. Never rewrites. Stamps `jobs.details_added_at` |
| עוד לא הגיעו הצעות | `warn_quiet_jobs()` · `no_bids_yet` | One nudge, thirty minutes into a call with no offer, never for a call older than a day. Stamp: `jobs.quiet_warned_at` |
| חלון הגעה | `bids.arrival_window_start` / `arrival_window_end` · `lib/validation/arrivalWindow.ts` | The hours a pro commits to. Required for `today`/`tomorrow`, at most four hours, never past (`bids_check_arrival_window`). Two-hour slots 08–20, Israel time |
| תזכורת ביקור | `remind_todays_visits()` · `visit_reminder` | Both sides, once, from 07:00 Israel time on the day of the window. Stamp: `jobs.visit_reminded_at` |
...
| הקישור האישי | `PersonalLinkCard` · `/new-request?pro=<slug>` | On `/pro/profile`: the link, a server-drawn QR (`qrcode`) and a WhatsApp share. `verified_pro_id_by_slug()` resolves it |
| תיק הבית | `my_home_record()` · `/account/home` | Finished jobs by the nearest saved address within 150 m. A definer, because the pro's name and slug live in closed tables |
| זמן תגובה | `pro_response_minutes()` | Average minutes from posting to a pro's offers, over at least three; null below. No client grant — reached through `bids_for_job()` |
| הצצה מהירה | `loadProPeek()` · `ProPeekButton` | The public profile and three latest reviews in a panel over the compare screen. The Phase 8 definer functions, nothing new exposed |
| הזמנה חוזרת | `CUSTOMER_ROUTES.newRequestTo(slug)` | A call directed at a saved pro, from `/account`, the posting form or the home record |
| תזכורת דירוג | `remind_unreviewed_jobs()` · `review_reminder` | Once, three hours after a job closes and within its first week. Stamp: `jobs.review_reminded_at` |
...
/app                    → Next.js App Router routes
  ...
  /(customer)            → customer-facing routes — AT THE ROOT: /login, /account,
                            ...
                            /new-request sits OUTSIDE (authed) since Phase 13.7:
                            the phone is asked for at publish, not at the door
...
/lib
  ...
  /maps                    → geometry.ts (the leaf: bounds, haversine, EWKT),
                             gazetteer.ts (the matcher), localities.data.ts
                             (GENERATED — npm run gazetteer:build) and
                             geocode.ts, which layers Google and the device's
                             own point on top of them
...
/assets/fonts              → the Heebo TTF faces the PDF embeds. Not `public/`:
                             ...
                             `fs` at request time
/scripts                   → operational tools, run by hand, never in a build.
                             build-gazetteer.mjs regenerates the locality list
                             from Wikidata; its output is committed
/supabase
...
- [ ] **A real Google Maps Platform key — deliberately not taken for now** (decided with the user, 9.9.2026). A key is effectively free at this volume (one geocode per posted call, one per pro profile save, against a monthly free tier in the thousands) but it **requires a billing account with a card**, and it makes the address path depend on a third party that can expire, rate-limit or refuse. What the product needs today is which town a call is in, so that `pro_profiles.radius_km` selects the right pros — and a 10–30 km radius does not care about house numbers. So the gazetteer is the supported path (section 2), and it is honest about being locality-level: a job placed from it is flagged `approximate` and the screen says so. **What is still missing without a key** is street-level accuracy, which is what turns "the right town" into "the right door" — Places Autocomplete for the customer, and a pin a pro can navigate to. The device's own GPS closes most of that gap for free and is what `AddressField` offers first. This entry closes the day somebody needs door-level navigation, not before
- [x] **Push notifications — built in Phase 13, on free channels, behind an adapter.** Phase 10 made the product depend on something that did not exist: a customer's choice expires in two hours, `accept_job()` charges 35 ₪ before the work and never refunds it, and nothing told the pro they had been chosen. What was built (decided with the user, 10.9.2026): a `notifications` table, the in-app centre the `pro-5.4` design had been waiting on, and Web Push — **with the send layer behind `lib/notifications/provider.ts`**, so buying an Israeli SMS gateway later is one file (`providers/sms.ts`) and one line, with no migration, no screen and no second copy of the Hebrew. `costsMoney` on the provider is what makes that a flag rather than an edit, the same lever `AUTH_BYPASS_OTP` is. **The governing rule, and why this does not weaken section 2's "nothing in the product may depend on a sweep having run":** the row is written *inside the transaction that caused it* — `select_bid()` writes it beside the `update` that moves the job to `awaiting_pro` — and the push is a best-effort accelerant on top. `/pro/notifications` is correct with every sweep switched off; the sweep only ever stamps `pushed_at`. `pg_net` turned out to be available after all, so the dispatcher is pg_cron → `pg_net` → a route handler that holds **no database credential** (the batch carries everything a channel needs), which is what keeps `SUPABASE_SERVICE_ROLE_KEY` unread anywhere in this repo. **What this does NOT solve, and is why the entry below it stays open:** a person who declines the permission prompt gets nothing, and iOS delivers push only from a home-screen install. Those are exactly where SMS money would go, and section 8 keeps that with the user — Twilio is $0.2575 per SMS to Israel, a local gateway roughly a tenth of that
- [ ] **A paid notification channel, or none.** Phase 13's adapter exists so this is a decision and not a project, and it is now the *only* thing standing between the product and "the pro was told" being reliable. Web Push covers a pro who granted permission on Android or desktop, and a pro who installed the site on an iPhone. It covers nobody who said no, and nobody on iOS who did not install. When this is taken: write `lib/notifications/providers/sms.ts`, add it to `PROVIDERS`, set `NOTIFICATIONS_ALLOW_PAID=1`. `URGENT_KINDS` in `lib/notifications/kinds.ts` already answers the question that decision will raise — `bid_selected` and `price_update_requested`, the two events where money and a clock are both running, and the only two worth paying per message for
...
- [x] **Cancelling a job — decided by the user on 15.9.2026, built in Phase 15.** A customer cancels on their own until a pro accepts; after that it goes through the pro ("הלקוח ביטל") or the admin, and the pro receives a credit that makes their next accepted job free — the fee is still never refunded (8.9.2026). A job the pro was not charged for creates no credit. The customer is told when a pro cancels, and can dispute it, because a pro who could cancel silently could take a job off the platform and claim the credit. No-shows are **not** built (the user's call): they stay a support matter. Fold into section 3 in the commit that builds it, and remove this entry then
...
- [ ] **An AI model in the job-posting path, or none.** Opened on 14.9.2026: the user is open to a photo → category + draft description step, subject to cost. The estimate before measuring is roughly 2 agorot per call on Claude Haiku 4.5, 4–5 on Claude Sonnet 5 and 10–15 on Claude Opus 5 — under one percent of the fee even at the top, so cost is unlikely to be what decides. What will decide is a third party in the critical path, a photo from inside somebody's home leaving for a processor, and an anonymous visitor (Phase 13.7 opens the form) running up calls. The answer was meant to be a hand-run measurement, `scripts/photo-intake-spike.mjs`, on 20 real photos, never wired into the product — **deferred by the user on 14.9.2026**, so Phase 13.8 does not build it and this entry stays open. If it is taken, it is a section 2 row, a flag, explicit consent before the photo is sent, a rate limit, a form that works unchanged when the service is down, and a clause in `lib/content/legal.ts`; the model only ever **fills** the form, it never publishes
