...
- **A job reaches a pro inside the pro's own radius, and that is the whole rule** — `pro_serves_job(location)`, enforced in the RLS policy on `jobs`, not in the feed query, so it holds for anything that ever reads the table. It used to be `least(pro_profiles.radius_km, jobs.search_radius_km)` and the customer picked their half on the posting form. That half was removed on 11.9.2026, column and all: a customer does not know how far a plumber will drive, has no way to find out, and a number they guessed was quietly narrowing their own market. A pro who has set 30 km has already answered the question, for themselves, with information the customer does not have. **The customer therefore has no lever over reach at all** — which is why the offers screen, when nobody covers the address, states the fact and offers no button.
...
- [x] **Waiving the 35 ₪ fee on a customer a pro brought in themselves — decided by the user on 14.9.2026, built in Phase 13.8.** The argument for it: a pro is being asked to hand Handy a customer who was already theirs, and a fee on that customer is the reason they would not. The rule: **a new customer, their first job, taken by the pro whose link they came through** (`jobs.requested_pro_id`). "New" means no earlier job of theirs was ever accepted by anybody — which is also what stops it being gamed, because a customer a pro met through the feed is no longer new by the time the link could reach them. The lever is `job_acceptance_fee()` taking the job and the pro, with `accept_job()` still reading the amount from it and never from the caller. Two companion decisions made the same day, both of which rewrite a section 3 rule when they land: a directed job stays the requested pro's **until that pro passes** (or the customer opens it to everyone), and it reaches that pro **even outside their radius**, because the customer named them — the radius decides what the feed brings, not who a customer may ask for. Fold all three into section 3 in the commit that builds them, and remove this entry then
