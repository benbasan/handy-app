...
- [ ] **Push notifications — the approach is now decided, and the build is Phase 13.** Phase 4 needed none, and Phase 5 still did not: the tracking screen, the price-update card and both chats update through Supabase Realtime while the tab is open. That remains a different thing from reaching someone who has closed it — Phase 5 made the gap visible, and **Phase 10 made the product depend on it**: a customer's choice now expires in two hours unless the pro answers, and nothing outside an open tab tells them they were chosen. The two-hour window was chosen to survive that, not to excuse it. **Decided with the user on 10.9.2026, after the end-to-end product review: free channels only for now** — a `notifications` table, an in-app notification centre (the `pro-5.4-notifications` screen designed and never built), and Web Push — **with the send layer written behind a provider adapter**, so that buying an Israeli SMS gateway later is one file (`lib/notifications/providers/sms.ts`) and one line, not a rewrite. That is the same seam `lib/observability.ts` is for a monitoring vendor and `lib/auth/bypass.ts` is for a paid channel stood down. **The governing rule, and the reason this does not weaken section 3's "nothing in the product may depend on a sweep having run":** the notification row is written *inside the transaction that caused it* — `select_bid()` writes it beside the `update` that moves the job to `awaiting_pro` — and the push is a best-effort accelerant on top. No transport can make "the pro was told" an invariant: a browser can be offline, the permission revoked, the endpoint gone. **What this entry still does not close** is a pro who never grants permission, and iOS, which delivers push only from a home-screen install — those are exactly where SMS money would go, and section 8 keeps that with the user. Twilio remains priced at $0.2575 per SMS to Israel; a local gateway is roughly a tenth of that. **One thing to verify before writing the migration:** `pg_net` is not installed today (only `postgis` and `pg_cron` are), and the recommended dispatcher depends on it — the fallback, Vercel Cron calling a route handler, would need `SUPABASE_SERVICE_ROLE_KEY`, which no code in this repo reads today
