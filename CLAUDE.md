...
- **Adding to a call appends; it never rewrites.** `add_job_details()` works only while a call is `open`/`bidding` and adds after what pros already read, because an offer made against one description must not come to mean another. The Phase 1 UPDATE grant on `jobs.description` and `photo_urls` is still there and is not what the product offers — revoking it is recorded in the roadmap as debt rather than done in passing.
