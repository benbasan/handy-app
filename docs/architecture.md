...
| שינוי | למה |
|---|---|
| **`job_fees.pro_id` נוסף** (הטבלה נקראה `commission_charges` עד Phase 10) | בטיוטה בעל המקצוע נגזר דרך `job → selected_bid → pro`. מדיניות RLS של "בעל מקצוע רואה רק את ההכנסות שלו" הייתה הופכת ל-join דו-שלבי שמורץ על כל שורה — גם איטי וגם קשה לביקורת. עמודה מפורשת = השוואת עמודה אחת מאונדקסת. |
...
| שינוי | למה |
|---|---|
| ~~**`jobs.search_radius_km`**~~ | **נמחקה 11.9.2026.** הלקוח בחר בה רדיוס שידור, והיא הצטלבה עם הרדיוס של בעל המקצוע ב-`least()`. הוסרה על העמודה עצמה: לקוח אינו יודע עד כמה בעל מקצוע מוכן לנסוע, ומספר שניחש צמצם לו את השוק. השער היחיד הוא `pro_serves_job(location)` — `pro_profiles.radius_km` בלבד. |
...
| **`pro_serves_job(point)`** | הייתה `pro_serves_job(point, search_radius_km)` עד 11.9.2026, ולפני כן `pro_serves_point(point)`. חזרה לארגומנט אחד כשחצי הלקוח בכלל ההגעה בוטל. |
...
**רדיוס אחד, של בעל המקצוע.** ~~שני הרדיוסים חייבים להסכים~~ — כך זה היה מ-Phase 3 ועד 11.9.2026, כש-`jobs.search_radius_km` נמחקה. קריאה נראית לבעל מקצוע רק במרחק `pro_profiles.radius_km` שלו עצמו. האכיפה נשארה **במדיניות ה-RLS** ולא בשאילתה, כדי שתחול על כל דבר שיקרא אי פעם את הטבלה — כולל `can_read_job_media`. `supabase/tests/rls_test.sql` מוכיח את זה בקריאה אחת שישה ק״מ מבעל המקצוע: נראית ברדיוס 10, נעלמת כשהוא מצמצם ל-3, וחוזרת כשהוא מרחיב בחזרה.
...
| **`select_bid(bid_id)`** — `security definer`, ו-`jobs` איבד `update (status, selected_bid_id)` | זהו מסלול כסף: המחיר המוסכם של קריאה הוא מחיר ההצעה שנבחרה, ולכן קליינט שיכול לכתוב `selected_bid_id` יכול לבחור הצעה שפג תוקפה, הצעה על קריאה של מישהו אחר, או לבחור מחדש בדיעבד. הפונקציה בודקת בעלות, שהקריאה עדיין פתוחה ושההצעה לא פגה. **שונה ב-Phase 10:** היא כבר לא נועלת את היריבות ולא כותבת `selected_bid_id` — זה עבר ל-`accept_job()`. |
...
### מה נוסף ב-Phase 10 (`supabase/migrations/20260911120000_pro_acceptance_and_flat_fee.sql`)

שני שינויים במודל העסקי שנפגשים בנקודה אחת, ולכן מיגרציה אחת: בחירת הלקוח הפכה ל**הצעה** שבעל המקצוע צריך לאשר, והעמלה של 12% מעבודה שנסגרה הפכה ל**סכום קבוע של 35 ₪ שנגבה ברגע האישור**. הרגע שבו נכנס כסף עבר מסוף הזרימה לתחילתה.

| שינוי | למה |
|---|---|
| **`select_bid()` כבר לא משבצת** — היא מסמנת `selected`, חותמת `accept_deadline` של שעתיים ומעבירה את הקריאה ל-`awaiting_pro` | עד שבעל המקצוע ענה אין הסכמה. היא במפורש **לא** כותבת `selected_bid_id` ו**לא** דוחה את היריבות: הקריאה נשארת בפיד, ממשיכה לקבל הצעות, והלקוח יכול להעביר אותה למישהו אחר (`select_bid()` שוב) או לקחת אותה בחזרה (`withdraw_bid_selection()`). זה מה שהופך חלון של שעתיים לנסבל מצד הלקוח |
| **`accept_job(bid_id)`** — `security definer`, אידמפוטנטית | כל מה ש-`select_bid()` עשתה בסוף עברה לכאן, ועוד החיוב: משבצת, דוחה כל יריבה, וכותבת שורת `job_fees` — הצהרה אחת, כי כל חלק בה חייב להיות נכון באותו רגע. בודקת `is_verified_pro()` מחדש, כי בעל מקצוע שהושעה בין ההצעה לתשובה אסור שייקח עבודה. אידמפוטנטית מאותה סיבה ש-`complete_job()` היא: זו לחיצה מטלפון, ובקשה שנשלחה פעמיים אסור שתחייב פעמיים |
| **`selected_bid_id` נכתב רק באישור** | `is_assigned_pro()` מוגדרת דרך העמודה הזו, ודרכה מדיניות הצ׳אט, המיקום החי, `request_price_update()`, `complete_job()` והקבלה. כתיבה שלה בזמן הבחירה הייתה מעניקה את כל אלה לבעל מקצוע שמעולם לא ענה. אף אחת מהפונקציות האלה לא השתנתה במיגרציה הזו — וזו הסיבה |
| **`decline_job()` / `withdraw_bid_selection()` / `expire_stale_selections()`** | שלוש הדרכים שבהן הצעה נגמרת בלי תשובה חיובית. אף אחת לא גובה דבר, ובכולן הקריאה חוזרת ל-`bidding` עם ההצעות המתחרות שלמות |
| **`commission_charges` → `job_fees`**, ו-`total_price`/`payment_method`/`completed_at` הפכו nullable | זו אותה עובדה שנמדדת אחרת, ולכן שינוי שם ולא טבלה שנייה — שתי טבלאות היו שתי תשובות לשאלה כמה העבודה הזו הכניסה. השורה נולדת באישור ומושלמת בסגירה, ולכן כל קורא שמתכוון ל"עבודה שהסתיימה" חייב לומר זאת: `pricing_guide()` ו-`category_stats()` הן החוד — בלי הסינון, המחיר שזר קורא ב-`/pricing` היה כולל עבודות שהמחיר שלהן עדיין לא ידוע, כ-null |
| **`job_acceptance_fee()`** — `immutable`, מחזירה 35 | אותה סיבה בדיוק ש-`commission_rate()` הייתה פונקציה: המספר מופיע בכרטיס האישור, בקבלה, במסך ההצעה ובארנק. טסט Vitest קורא את המיגרציה ומוודא שהקבוע ב-`lib/validation/pros.ts` זהה לו |
| **הדמים אינם מוחזרים, אף פעם** | הכרעת המשתמש (8.9.2026). לכן `resolve_dispute()` לא מופיעה במיגרציה הזו בכלל: זיכוי ללקוח ודמים שבעל המקצוע שילם כדי לקחת את העבודה הם שתי שאלות, ולמוצר הזה יש תשובה רק לראשונה |
| **`bid_effective_status()`** | Phase 4 שכפלה `case when status = 'pending' and expires_at <= now()` לארבע פונקציות קריאה. עם שני שעונים, ארבעה עותקים של case דו-זרועי הם הדרך שבהם הם מתפצלים |

**מה שלא נבנה ב-Phase 10 בכוונה:** אין push ואין SMS, והמודל החדש תלוי בכך שבעל המקצוע ישים לב תוך שעתיים. חלון השעתיים נבחר כדי לשרוד את זה, לא כדי לתרץ אותו — הפער רשום ב-`CLAUDE.md` סעיף 9 והוא כעת חסם מוצרי ולא נוחות. באותו אופן: ביטול עבודה משובצת עדיין אינו קיים, ועכשיו הוא עולה לבעל המקצוע 35 ₪ ולא רק זמן.
...
> **שונה ב-Phase 13.7:** `/new-request` עצמו כבר אינו מוגן — הטופס פתוח לכולם, ומספר הטלפון נשאל בלחיצה על "פרסם". ב-`PROTECTED_AREAS` נשאר רק `/new-request/published`. מי שכותב ל-`jobs` הוא עדיין `createJob`, שדורש לקוח מחובר.
...
- **`bids.status`** כולל `pending / selected / accepted / declined / rejected / expired` (`accepted` ו-`declined` נוספו ב-Phase 10, ו-`selected` שינה משמעות ל"הלקוח בחר, בעל המקצוע טרם ענה"). לכל הצעה **שני שעונים ולא אחד**: `expires_at` — 45 דקות עד שמישהו בוחר, ו-`accept_deadline` — שעתיים לענות אחרי שנבחרה. הם לעולם לא רצים יחד: `expire_stale_bids()` נוגעת רק ב-`pending` ו-`expire_stale_selections()` רק ב-`selected`, שתיהן ב-**pg_cron** כל דקה (ולא Edge Function — cron במסד הנתונים לא דורש יעד דיפלוי) ובנוסף נקראות מהמסכים. הנכונות אינה תלויה בהן: `select_bid()` ו-`accept_job()` קוראות את השעון בעצמן, ו-`bid_effective_status()` מדווחת הצעה שפגה כ-`expired` בכל מקרה.
...
- **`job_fees`** (נקראה `commission_charges` עד Phase 10) נכתבת **פעמיים ועל ידי שתי פונקציות**: `accept_job()` פותחת אותה ברגע שבעל המקצוע לוקח את העבודה — `base_price` מההצעה שאישר ו-`fee_amount` מ-`job_acceptance_fee()` — ו-`complete_job()` משלימה אותה בסגירה עם `total_price` מ-`job_effective_price()`, אמצעי התשלום ו-`completed_at`. לכן שלוש העמודות האחרונות **null בעבודה שנלקחה ועדיין רצה**, וכל קורא שמתכוון ל"עבודה שהסתיימה" מסנן על `completed_at`. לאף תפקיד קליינט אין INSERT או UPDATE עליה.
...
- **`jobs`**: לקוח רואה/עורך רק קריאות שהוא יצר. בעל מקצוע רואה קריאות בסטטוס `open`/`bidding`/`awaiting_pro` שבתחום/רדיוס שלו, ורואה קריאות שהגיש עליהן הצעה או שזכה בהן. אדמין רואה הכל. (`awaiting_pro` נכנס ב-Phase 10: קריאה שנבחרה ולא אושרה עדיין אינה מחויבת לאיש, ולכן נשארת בפיד.)
...
- **`job_fees`**: בעל מקצוע רואה רק את שלו. אדמין רואה הכל. לקוח לא רואה בכלל (זה לא עניינו) — גם לא את שורת העבודה שלו עצמו, ולכן `job_receipt()` מחזירה לו את הסכום ששילם בלי דמי קבלת העבודה.
