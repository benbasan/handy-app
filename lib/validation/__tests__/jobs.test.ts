...
describe("createJobSchema", () => {
  ...
  /*
   * There used to be a "rejects a radius that is not one of the offered
   * options" case here. The customer stopped having a radius on 11.9.2026, so
   * the schema has no field to reject — a call reaches every pro whose own
   * radius_km covers the address, which is a question only the pro can answer.
   * The assertion that replaces it is in pgTAP, against the policy.
   */
